...
from geoworker import mc, db, SERVICE_NAME
from geoworker.tags import CLIFF_CLAVIN_2_3_0_TAG_ID
import geoworker.tasks
...
for story in stories_with_text:
    already_tagged = CLIFF_CLAVIN_2_3_0_TAG_ID in story['story_tags']
    if retag or not already_tagged:
        logger.debug("       queued story {}".format(story['stories_id']))
        geoworker.tasks.geocode_from_story_text.delay(story)
        queued += 1
    else:
        already_tagged += 1
